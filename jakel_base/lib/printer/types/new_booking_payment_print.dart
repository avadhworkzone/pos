...
Future<bool> printNewBookingPayment(
    String? title, BookingPayments sale, bool duplicateCopy) async {
  ...
  widgets.addAll(getCashierDetails(sale.counter ?? user?.counter,
      sale.cashier ?? user?.cashier)); // Cashier Details
  ...
  widgets.addAll(getCustomerDetails(sale.customer)); //  Customer Details
...
List<pw.Widget> getInvoice(BookingPayments sale) {
...
pw.Widget getPaymentTypes(BookingPayments sale) {
  return pw.Table(
...
List<pw.TableRow> getPaymentTypesRow(BookingPayments sale) {
  List<pw.TableRow> tableRows = List.empty(growable: true);
...
List<pw.Widget> promotersWidget(BookingPayments sale) {
  List<pw.Widget> widgets = List.empty(growable: true);
  widgets.add(mySeparator());
