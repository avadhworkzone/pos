...
class PostStoreManagerProfileUpdateRepository {
  ...
  Future<dynamic> fetchPostStoreManagerProfileUpdate(
      dynamic mPostStoreManagerProfileUpdateListRequest) async {
    final response = await webservice.postWithAuthAndRequest(
        WebConstants.actionPostStoreManagerProfileUpdate,
        mPostStoreManagerProfileUpdateListRequest);
    try {
      WebCommonResponse mWebCommonResponse =
          WebCommonResponse.fromJson(response);
      if (mWebCommonResponse.statusCode.toString() == "200") {
        PostStoreManagerProfileUpdateResponse mWebResponseErrorMessage =
        PostStoreManagerProfileUpdateResponse.fromJson(json.decode(mWebCommonResponse.data));
