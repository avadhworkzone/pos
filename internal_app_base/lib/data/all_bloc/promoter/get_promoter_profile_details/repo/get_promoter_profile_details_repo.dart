...
class GetPromoteProfileDetailsRepository {
  final Webservice webservice;
  final SharedPrefs sharedPrefs;
  dynamic returnResponse;

  GetPromoteProfileDetailsRepository({required this.webservice, required this.sharedPrefs});

  Future<dynamic> fetchGetPromoteProfileDetails(dynamic mGetPromoteProfileDetailsRequest) async {
    final response = await webservice
        .getWithAuthAndStringRequest(WebConstants.actionGetPromoteProfileDetails,mGetPromoteProfileDetailsRequest);

    try {
      WebCommonResponse mWebCommonResponse =
          WebCommonResponse.fromJson(response);
      if (mWebCommonResponse.statusCode.toString() == "200") {
        GetPromoteProfileDetailsResponse dashboardResponse =
            GetPromoteProfileDetailsResponse.fromJson(json.decode(mWebCommonResponse.data));
