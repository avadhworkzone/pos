...
class AmountBuyForXGetYFlatOff with BasePromotion {
  final tag = "AmountBuyForXGetYFlatOff";

  @override
  String samples() {
    return "ITEM_WISE_AS_PER_AMOUNT_GET_OFF_ON_OTHERS_FLAT";
  }

  @override
  String title() {
    return "Buy “X” amount from the Buy Products list and get “X” Flat amount Off from the uploaded get products list.	";
  }
  ...
  List<CartItem> applyDiscount(CartSummary cartSummary, Promotions promotions) {
    ...
    MyLogUtils.logDebug(
        "$tag, applyDiscount discountPercentage $flatDiscountAmount & thresholdAmount :$thresholdAmount");
    ...
    if (flatDiscountAmount > 0) {
      ...
      cartSummary.cartItems?.forEach((element) {
        if (_isAvailableInGetProducts(promotions, element)) {
          ...
          MyLogUtils.logDebug(
              "$tag discountPercentage is : $discountPercentage");
