...
class VcTierCustomerPercentageMultiple with BaseVoucherConfig {
  final tag = "VcTierCustomerPercentageMultiple";
  ...
  double getVoucherAmount(
    ...
    if (isValidForThisCart(cartSummary, voucherConfiguration)) {
      ...
      for (var element in tier) {
        total = total +
            getDoubleValue(getTotalForVoucherGenerationValidation(cartSummary) *
  ...
  List<VoucherConfigTiers> _getTier(
    ...
    while (checkForTiers) {
      ...
      configTier.percentage = voucherConfiguration.percentage;
  ...
  List<SaleVoucherConfigs>? getSaleVoucherConfig(
    ...
    for (var element in tier) {
      configs.add(SaleVoucherConfigs(
          ...
          percentage: element.percentage ?? 0.0,
          flatAmount: null,
