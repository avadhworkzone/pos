...
class GiftCardLayawayPaymentWidget extends StatefulWidget {
  ...
  const GiftCardLayawayPaymentWidget({
    Key? key,
  ...
  State<StatefulWidget> createState() {
    return _GiftCardLayawayPaymentState();
  }
}

class _GiftCardLayawayPaymentState extends State<GiftCardLayawayPaymentWidget> {
  bool callApi = false;
  ...
  void _updateAmountToBeUsed() {
    if (widget.pendingAmount > (selectedGiftCard?.availableAmount ?? 0)) {
      amountTobeUsedFromGiftCard = (selectedGiftCard?.availableAmount ?? 0);
    }

    if ((selectedGiftCard?.availableAmount ?? 0) >= widget.pendingAmount) {
      amountTobeUsedFromGiftCard = widget.pendingAmount;
    }
  }
