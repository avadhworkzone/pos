...
class SaleReturnsListWidget extends StatefulWidget {
  final Function onSelected;

  const SaleReturnsListWidget({Key? key, required this.onSelected})
      : super(key: key);

  @override
  State<StatefulWidget> createState() {
    return _SaleReturnsListState();
  }
}

class _SaleReturnsListState extends State<SaleReturnsListWidget> {
  ...
  void _getSales() {
    MyLogUtils.logDebug("_getSales searchText : $searchText");

    if (searchText != null && searchText!.isNotEmpty) {
      viewModel.getSaleReturnsHistory(pageNo, perPage,0,0,searchText);
    } else {
      viewModel.getSaleReturnsHistory(pageNo, perPage, 0, 0,null);
    }
  }
    ...
    return StreamBuilder<SalesResponse>(
      ...
      builder: (context, snapshot) {
        ...
        if (snapshot.connectionState == ConnectionState.active) {
          ...
          if (responseData != null && responseData.saleReturns != null) {
            salesResponse = responseData;
            sales = responseData.saleReturns;
      ...
      dataRows.add(
        DataRow(
          ...
          cells: <DataCell>[
            ...
            DataCell(Align(
              alignment: Alignment.centerLeft,
              child: Text("${sale.offlineSaleReturnId}",
                  style: Theme.of(context).textTheme.bodyMedium),
            )),
            DataCell(
              Align(
            ...
            DataCell(
              Text(viewModel.getTotalReturnsItems(sale),
                  style: Theme.of(context).textTheme.bodyMedium),
            ),
            DataCell(
              Text(viewModel.getTotalReturnQuantities(sale),
                  style: Theme.of(context).textTheme.bodyMedium),
            ),
            DataCell(
              Align(
